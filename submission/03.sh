# What is the address of the P2SH script wrapping the witness redeem script in this transaction
# transaction=""
# Derivation: The witness redeem script is the same 2-of-2 multisig as in 02. For P2SH-wrapped SegWit (P2SH-P2WSH),
# the redeem script is OP_0 + 32-byte witness script hash (SHA256 of the witness redeem script). That scriptPubKey
# is then treated as a P2SH redeem script; bitcoin-cli getaddressinfo / decodescript / addwitnessaddress-style flow
# on regtest yields the Base58Check P2SH address below.
echo 2NByn92W1vH5oQC1daY69F5sU7PEStKKQBR
